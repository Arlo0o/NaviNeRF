...
class Trainer(object):
    ...
    def train(self, G,  deformator, shift_predictor, multi_gpu=False, skipping_G =None):
        G.cuda().eval()
    ...
    ###########################################
        ...
        for step in range(recovered_step, self.p.n_steps, 1):
            ...
            z = z.squeeze()
            # print("shape of z:",  z.shape, )  ###  32 512 1 1
            ...
            if should_gen_classes:
                ...
                imgs = G(z)
                imgs_shifted = G.gen_shifted(z, shift)  ### 32 3 256 256
            ...
            if skipping_G is not None:
                    skip_imgs = skipping_G(z)
                    skip_imgs_shifted = skipping_G.gen_shifted(z, shift)
                    output0 = (skip_imgs.permute(0,2,3,1)[0].squeeze().data.cpu().numpy()*256 ).astype('uint16')
                    imgs=skip_imgs
                    imgs_shifted=imgs_shifted

                    logits_skip, shift_prediction_skip = shift_predictor(skip_imgs, skip_imgs_shifted)
                    logit_loss_skip = self.p.label_weight * self.cross_entropy(logits_skip, target_indices)
                    shift_loss_skip = self.p.shift_weight * torch.mean(torch.abs(shift_prediction_skip - shifts))

                    external_logit_loss = F.mse_loss(logits, logits_skip)
                    external_shift_loss = F.mse_loss(shift_prediction, shift_prediction_skip)

                    loss =  logit_loss + shift_loss   + external_logit_loss + external_shift_loss   + logit_loss_skip + shift_loss_skip 
