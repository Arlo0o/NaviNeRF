...
@torch.no_grad()
def interpolate(G, z, shifts_r, shifts_count, dim, deformator=None, with_central_border=False, index0= None, index=None):
    shifted_images = []
    all_zs=torch.zeros(7,512).cuda()
    i=0

    for shift in np.arange(-shifts_r, shifts_r + 1e-9, shifts_count):
